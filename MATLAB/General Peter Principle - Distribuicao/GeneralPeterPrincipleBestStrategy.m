function [efficiencyvector,distribuition] = GeneralPeterPrincipleBestStrategy(hierarchy,pyramid,delta,timestep)
global levels
emptyList=[0,0];
range=1:timestep;
eff(range)=0;
...
for t=range(2:end)
    ...
    while(~all(emptyList==0))%%%
        ...
        if emptyList(cont,1)~=levels
            ...
            indexChosen=BestStrat(hierarchy,pyramid,emptyList(cont,:));
...
for i=1:levels
    distribuition(i,1:sizebin)=distribuition(i,1:sizebin)/pyramid.position(i);
end
